...
from plots import plot_error_analysis, plot_confusion_metrics, plot_logistic_regression, save_dataframes
...
#full set
#LANGUAGES = ["es", "fr", "it", "no", "sv", "ar", "ru", "uk"]
...
def load_processed_dataset(model_name: str, languages: list[str]) -> DEvalDataset:
    ...
    # Read only headers to check which columns exist
    df_columns = list(pd.read_csv(processed_file, nrows=0, sep=";").columns)

    # Check which translation and x_gender columns exist
...
if __name__ == "__main__":
    ...
    for model_name in model_names:
        ...
        # Load dataset and get available languages
        ds, available_languages = load_processed_dataset(model_name, LANGUAGES)

        # Create folder for this model inside outputs/
        ...
        error_df.to_csv(os.path.join(model_output_dir, error_df.attrs["filename"] + ".csv"), sep=";", index=False)
        ...
        cm_df.to_csv(os.path.join(model_output_dir, cm_df.attrs["filename"] + ".csv"), sep=";", index=False)

        # -------------------------------
        # 3. Logistic Regression
        # -------------------------------
        lr_results = LogisticRegressionAnalysis(ds, "x_gender").analyze(predictor_col="x_stereotypical")
        lr_results.attrs["filename"] = f"{model_name}_logistic_regression"
        lr_results.to_csv(os.path.join(model_output_dir, lr_results.attrs["filename"] + ".csv"), sep=";", index=False)

        # -------------------------------
        # Save plots
        # -------------------------------
        plot_error_analysis(error_df.T, output_dir=model_output_dir, filename=error_df.attrs["filename"])
        plot_confusion_metrics(cm_df.T, output_dir=model_output_dir, filename=cm_df.attrs["filename"])
        plot_logistic_regression(lr_results, output_dir=model_output_dir, filename=lr_results.attrs["filename"])

        # Optionally save all dataframes together
