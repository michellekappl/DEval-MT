...
class DEvalDataset:
    ...
    def clone(self) -> 'DEvalDataset':
        ...
        Copies the underlying DataFrame, translation column mapping, and
        prediction column mapping so that downstream pipeline steps can add
        columns without mutating the original object passed by the caller.
        """
        new = DEvalDataset(self.df.copy(deep=True), self.text_column)
        new.translation_columns = self.translation_columns.copy()
        new.prediction_columns = self.prediction_columns.copy()
        new.path = self.path
        return new
